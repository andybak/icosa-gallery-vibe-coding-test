...
If GitHub Pages loads but shows a blank page after a successful deploy, it's usually stale service-worker cache from an older build.

Fix in browser:
1. Open DevTools → Application → Service Workers.
2. Click **Unregister** for this site.
3. Clear site data/storage cache.
4. Hard refresh the page.

This repo's service worker now uses versioned caches and a network-first strategy for navigations to reduce stale-HTML issues between deploys.
